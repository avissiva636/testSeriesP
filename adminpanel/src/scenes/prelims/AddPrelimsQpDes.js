...
import { useCreatePSeriesDesMutation, useGetBatchesQuery, useGetCoursesQuery, useGetOutlinesQuery, useGetSubjectsQuery } from 'state/apiDevelopmentSlice';
import InvisibleFileUploader from 'components/InvisibleFileUploader';

const AddPrelimsQpDes = () => {
    ...
    const [createPSeriesDes] = useCreatePSeriesDesMutation();
    ...
    const handleSubmit = async () => {
        ...
        formData.append('pSeries', pQDesId);
        ...
        try {
            await createPSeriesDes(formData).unwrap();
            handleReset();
        } catch (error) {
            if (error.status === 400) {
...
export default AddPrelimsQpDes